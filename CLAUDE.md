...
- **Pastilles** (`similaires.code.php`) appelle avec `max = $CS_MAX` + libellés
  **nettoyés** (`mt_sim_clean_label` : strip « Les meilleur(e)s… » + capitale). Le
  comparatif courant est ajouté **en tête, en plus** (accentué, hors `max`).
...
- **Bande** (fond gris `#fafbfc` + filet haut `1px #e8eaed` + padding vertical) =
  **portée par la SECTION Bricks en natif**, PAS dans le CSS du bloc (structure
...
- **Blocs Code (remplacent les blocs V1 dans le template dupliqué)** — sans
  ...
  - `multi-sommaire.code.php` : **titre** = « Meilleur » + type de produit,
    ...
    12px, **gris sombre `--at-grey`**, PAS noir) souligné d'une **bordure 2px de la
    même couleur** ;
